...
6. Use the [SQL script](https://github.com/SmartChico/Chico/blob/master/script.sql) in the Github repository to generate the `chico` database locally. Then run the [SQL initialization script](https://github.com/SmartChico/Chico/blob/master/initialData.sql) on the newly created database to initialize the database. To use the online development database or to get attachable (.mdf and .ldf) files for the test database, please contact the Chico team.
